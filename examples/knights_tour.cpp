...
// https://github.com/pearcej/cppds, sections 9.11-9.14 Otherwise authored by
// Jesse Walker
...
public:
  ...
  Vertex(const Vertex &copy)
      : id(copy.id), visited(copy.visited), connections(copy.connections) {}

  void addConnection(int id) {
    // ensure unique
    // the connections list is never going to be very large in this case
    if (std::find(connections.begin(), connections.end(), id) ==
        connections.end())
...
public:
  ...
  bool hasVertex(int id) { return vertices.find(id) != vertices.end(); }
  ...
  Vertex &operator[](int id) { return *vertices[id]; }
};
...
public:
  ...
  int getTileID(int x, int y) { return y * size + x; }
  ...
  void traverseTileIDs(ct::Turtle &t, std::list<int> &ids,
                       bool writePath = true) {
    if (ids.empty())
      return;
    t.penup();
...
std::list<int> tourIterative(Graph &g, Vertex &start) {
  ...
  while (path.size() < limit) {
    ...
    if (!curVertex.visited) { // Mark a vertex as visited if finding for the
                              // first time.
    ...
    if (curNeighbors.empty()) {
      ...
      // because it means that no path taken from this vertex with the frames
      // prior will lead to a solution.
      ...
      // Get the next neighbor in this frame's list and pop it from this frame's
      // list. This ensures that every time we come back to this vertex, we
      // progress to the next neighbor as a possibility.
      ...
      if (!neighborVertex.visited) { // If the vertex hasn't been visited...
        ...
        std::list<int> neighborNeighbors =
            warnsdorf(g, neighborVertex.connections);
        if (!neighborNeighbors.empty()) { // If there are possible moves, add
                                          // another stack frame!
...
std::list<int> tourTheKnight(ChessBoard &board, int startX = 1,
                             int startY = 1) {
  Graph graph;
  board.genKnightGraph(graph);
  std::list<int> path =
      tourIterative(graph, graph[board.getTileID(startX, startY)]);
  return path;
}
